...
function createRightSide() {
  ...
  iAddNoteIcon.addEventListener("click", () => {
    divInputContainer.style.display =
      divInputContainer.style.display === "block" ? "none" : "block";
    iAddNoteIcon.className =
      divInputContainer.style.display === "block"
        ? "fa-solid fa-xmark note-add-icon"
        : "fa-solid fa-square-plus note-add-icon";

    document.addEventListener("click", (event) => {
      if (
        !divInputContainer.contains(event.target) &&
        event.target !== iAddNoteIcon
      ) {
        divInputContainer.style.display = "none";
